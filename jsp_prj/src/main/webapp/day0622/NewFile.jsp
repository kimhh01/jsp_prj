...
<body>
  ...
  ================================================== -->
		...
		<div class="container marketing">
			...
			<jsp:include page="../fragments/bestProduct.jsp"></jsp:include>
			...
			<div>
			이름: <span><%=uDTO.getName() %></span>
			이메일: <span><%=uDTO.getEmail()%></span>
			나이: <span><%=uDTO.getAge()%></span>
			</div>
			<div>
			<!-- 1. 객체 생성 : 기본 생성자를 사용한 객체화 -->
			<jsp:useBean id="ud" class="day0622.UserDTO" scope="page"/>
			<!-- 2. setter method 호출 -->
			<jsp:setProperty property="name" name="ud" value="김김길동"/>
			<jsp:setProperty property="email" name="ud" value="kimkim@test.com"/>
			<jsp:setProperty property="age" name="ud" value="35"/>
			...
			id가 객체명이므로 script에서 expression에서 해당 객체를 사용할 수 있다. <br>
			<% String name = ud.getName(); %>
			<%=name %>, <%ud.getEmail(); %>

			</div>
			<jsp:include page="../fragments/product_list.jsp"></jsp:include>
			<!-- /END THE FEATURETTES -->
		</div>
		<!-- /.container -->
		<!-- FOOTER -->
