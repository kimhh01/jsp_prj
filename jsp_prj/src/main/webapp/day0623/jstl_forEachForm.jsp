...
<body>
  ...
  ================================================== -->
		...
		<div class="container marketing">
			<!-- <form 태그에 action속성을 정의하지 않으면 submit이 되었을 때 자신 페이지를 재 요청함 -->
			<form method="post">
			<label>이름</label>
			...
			<c:out value="${i.first or i.last}."/><span style="font-weight: bold">
			<c:out value="${subject }"/>
			<c:if test="${i.last or i.first }"></span></c:if>
			</c:forEach>
			<br>
			<button class="btn btn-success btn-sm">전송</button> <br>
			</form>
			...
			<table class="table table-hover">
				...
				%>
					<c:if test="${empty userData}">
						<tr>
						 <td colspan="4" style="text-align: center;"> 회원의 정보가 존재하지 않음</td>
						</tr>
					</c:if>
