...
<body>
  ...
  ================================================== -->
		...
		<div class="container marketing">
			...
			%>
			<div>
				<strong>EL에서는 직접 접근을 할 수 없다.</strong><br>
			...
			<!-- scope객체의 이름과 동일한 이름이 들어가 있을 때 scope객체명을 생략하면 접근이 
			작은 scope객체에 값을 가져다 사용한다. -->
			applicationScope : <span>${applicationScope.hour}(${hour})</span>
		<div>
			<strong>연산자</strong>
			${flag} / ${!flag} /${not flag}<br>
			${year} / ${year+1} / ${year%12 } / ${year mod 12 }<br>
			<!-- EL에서 제공하지 않는 연산자는 사용할 수 없음 -->
			${year > month }(${year gt month })
			${year < month }(${year lt month }) 
			${year >= month }(${year ge month }) 
			${year <= month }(${year le month }) 
			${year == month }(${year eq month }) 
			${year != month }(${year ne month }) 
			${year > month && day > month}(${year gt month and day gt month}) 
			${year > month || day > month}(${year gt month or day gt month}) 
